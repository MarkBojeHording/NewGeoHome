...
- **COMPLETED Express Repair/Upgrade System**: Successfully implemented express task report creation for repair/upgrade requests via FarmRadialMenu. Modified handleCreateExpressTaskReport to support both pickup and repair/upgrade task types. "REQUEST REPAIR" and "REQUEST UPGRADE" buttons now instantly create task reports without opening modals, matching the pickup functionality.
- **Enhanced Friendly Base UI**: Removed "Friendly" prefix from dropdown options (now shows "Main Base", "Flank Base", "Farm") and added green "[FRIENDLY]" label to top left corner of friendly base modal for clearer visual identification.
- **COMPLETED Non-Main Base Text Icons**: Updated map display for non-main friendly bases to show descriptive text labels instead of icons. Flank bases display "FLANK", farms show "FARM", boats show "BOAT", and garages show "GARAGE" within the circular map markers for improved clarity and identification.
