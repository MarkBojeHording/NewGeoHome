...
// Component to fetch and display reports for a specific player using centralized system
const PlayerReportsContent = ({ playerName }: { playerName: string | null }) => {
  const { data: allReports = [], isLoading } = useQuery({
    queryKey: ['/api/reports']
  })

  // Use the new centralized filtering system for player tags
  const playerReports = useFilteredReports(allReports, 'player', playerName || '')

  // Fallback: Also check legacy content fields for backward compatibility
  const legacyFilteredReports = allReports.filter(report => {
    if (!playerName) return false
    const content = report.content || {}
    
    // Handle comma-separated string format (legacy system)
  ...
  const combinedReports = [...playerReports]
  ...
  const handleOpenReport = (reportId: number) => {
    // TODO: Implement report modal/detail view
