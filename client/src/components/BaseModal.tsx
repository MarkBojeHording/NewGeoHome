        ...
        <div className="relative">
          ...
          <div className="bg-gray-800 rounded-lg shadow-xl mx-4 border border-gray-700 flex flex-col relative" style={{height: '95vh', maxHeight: '805px', width: '515px', zIndex: 50}}>
            ...
            <div className="flex-1 px-4 pt-4 space-y-3 overflow-y-auto text-gray-200" style={{paddingTop: modalType === 'enemy' ? '24px' : '12px', position: 'relative', zIndex: 1}}>
              {modalType === 'report' && (
                <div className="absolute border-2 border-yellow-400 bg-yellow-900/20 rounded-lg cursor-move" style={{top: "80px", left: "20px", width: "460px", zIndex: "50"}} draggable="true" onDragEnd={(e) => { const rect = e.currentTarget.getBoundingClientRect(); const modalRect = e.currentTarget.closest(".fixed")?.getBoundingClientRect(); if (modalRect) { const relativeTop = rect.top - modalRect.top; const relativeLeft = rect.left - modalRect.left; console.log("FINAL POSITION: top: " + relativeTop + "px, left: " + relativeLeft + "px"); } }} title="Drag me to position - check console for coordinates">
